...
本文档描述当前仓库的整体架构、核心数据流以及模块边界。当前实现是带后台管理、Postgres/SQLite 存储、单进程轮询的自托管版本。
...
3. **控制面存储层**：通过 `lib/storage/resolver.ts` 在直连 Postgres 与 SQLite 之间解析后端。
...
- `lib/core/poller.ts`：单进程轮询器，负责周期性执行健康检查。
- `lib/core/health-snapshot-service.ts`：统一检测、历史写入、Telegram 通知和时间线装配。
- `lib/core/official-status-poller.ts`：轮询 OpenAI / Anthropic 官方状态并做内存缓存。
- `lib/core/polling-config.ts`：统一管理轮询间隔、官方状态轮询间隔和并发默认值。

### 控制面存储层

- `lib/storage/resolver.ts`：解析 `postgres | sqlite`，并输出当前能力矩阵。
- `lib/storage/postgres.ts` / `lib/storage/sqlite.ts`：完整单实例后端，支持控制面、`check_history`、可用性统计聚合与自动建表。

### 管理与诊断层

- `lib/admin/auth.ts`：管理员账户、Session、Turnstile 集成。
- `lib/admin/data.ts`：后台数据读写聚合入口。
- `lib/admin/storage-diagnostics.ts`：存储能力矩阵与后端诊断信息。
...
|---|---|---|
| 管理员认证 | 支持 | 支持 |
| 检测配置 / 模板 / Telegram 推送 / 通知 / 站点设置 | 支持 | 支持 |
| 自动建表（控制面） | 支持 | 支持 |
| 历史快照写入 | 支持 | 支持 |
| 可用性统计 | 支持 | 支持 |
| 可配置站点图标 | 支持 | 支持 |

Postgres 适合正式自托管部署；SQLite 适合本地、自托管轻量部署或单机演示。当前不提供数据库租约选主、多实例去重或 active-active 同步。
...
- `lib/core/health-snapshot-service.ts` 的 `runProviderChecksAndPersist()` 是完整检测入口，会写历史并推进 Telegram 推送状态机。
...
- 可用性统计在 Postgres、SQLite 上都由历史记录派生。
...
- 当前不提供运行时迁移诊断 / 自动修复；Postgres 与 SQLite 会在启动时自动建表或补齐缺失列。
